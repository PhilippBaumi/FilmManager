            ...
            if (selectedSeries != null)
            {
                ...
                if (mainViewModel.Serien.Contains(selectedSeries))
                {
                    try
                    {
                        ...
                        await PreBakedMopupService.GetInstance().WrapTaskInLoader(tMDBService.DiscoverSerien(id, 1), Color.FromArgb("#1565C0"), Colors.Grey, new List<string> { $"{AppResources.page} 1 {AppResources.loading}"}, Colors.Black);
                        for (int page = discoversSeries.Page+1; page <= discoversSeries.TotalPages; page++)
                        {
                            ...
                            await PreBakedMopupService.GetInstance().WrapTaskInLoader(tMDBService.DiscoverSerien(id, page), Color.FromArgb("#1565C0"), Colors.Grey, new List<string> {$"{AppResources.page} {page} {AppResources.loading}"}, Colors.Black);
            ...
            if (selectedMovie != null)
            {
                ...
                if (mainViewModel.Movies.Contains(selectedMovie))
                {
                    try
                    {
                        ...
                        await PreBakedMopupService.GetInstance().WrapTaskInLoader(tMDBService.DiscoverMovies(id, 1), Color.FromArgb("#1565C0"), Colors.Grey, new List<string> { $"{AppResources.page} 1 {AppResources.loading}" }, Colors.Black);
                        for (int page = discoversMovies.Page+1; page <= discoversMovies.TotalPages; page++)
                        {
                            ...
                            await PreBakedMopupService.GetInstance().WrapTaskInLoader(tMDBService.DiscoverMovies(id, page), Color.FromArgb("#1565C0"), Colors.Grey, new List<string> { $"{AppResources.page} {page} {AppResources.loading}" }, Colors.Black);
