...
public partial class DetailPage : ContentPage, IQueryAttributable
{
    ...
    private async void HandleGenreChanged(object sender, EventArgs e)
    {
        ...
        if (selectedGenre is not null && this.tMDBService is not null && this.genreHelper is not null && o is Movie movie)
        {
            try
            {
                LoadingMoviesAndNavigateToOverview(selectedGenre);
            }
            catch
            {
        ...
        if (selectedGenre is not null && o is TvShow show && this.tMDBService is not null && this.genreHelper is not null)
        {
            try
            {
                LoadingTvShowsAndNavigateToOverview(selectedGenre);
            }
            catch
            {
                await Toast.ShowAsync(AppResources.cantNavigateToOverview, DialogType.Error);
    ...
    private async void LoadingTvShowsAndNavigateToOverview(string selectedGenre)
    {
    ...
    private async void LoadingMoviesAndNavigateToOverview(string selectedGenre)
    {
